...
#ifdef SATABS
int scull_init_module(); 
void scull_cleanup_module(void);
int scull_open(int, inode, file);
ssize_t scull_read(int, file, char, size_t, loff_t) ;
#define scull_release(i, filp) 0
ssize_t scull_write(int, file, char, size_t, loff_t);
...
inline loff_t scull_llseek(file filp, loff_t off, int whence)
{
  ...
  case 1: /* SEEK_CUR */
#ifdef SATABS
    newpos = filp + nondet() + off; //f_pos is undefined
#else
...
int main()
{
  __CPROVER_ASYNC_1: loader();
  __CPROVER_ASYNC_2: thread1();
  thread2();
}
#endif
