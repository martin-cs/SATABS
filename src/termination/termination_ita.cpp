    ...
    ranking_relation_cachet &ranking_relations)
{
  ...
  for(std::set<exprt>::const_iterator it=syms.begin(); it!=syms.end(); it++)
  {
    ...
    if (!has_prefix(id2string(name), "termination::pre::"))
    {
      ...
      new_sym.name = "ITA_SAVED::" + id2string(new_sym.name);
      new_sym.base_name = id2string(old_sym.base_name) + "''";
      new_sym.pretty_name = id2string(old_sym.pretty_name) + "''";
    ...
    {
      std::string n=id2string(name); // strip the pre-prefix
      n = n.substr(n.find("::") + 2);
      n = n.substr(n.find("::") + 2);
      n = n.substr(n.find("::") + 2);
