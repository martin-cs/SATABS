    ...
    switch(it->type)
    {
      case ASSIGN:
      {
            ...
            it++)
        {
          ...
          if(has_prefix(id2string(*it), termination_prefix))
          {
      ...
      case OTHER:
      {
        ...
        if(stmt=="decl")
          ...
          throw "Unknown `other' instruction: " + id2string(stmt);
      ...
      it++)
  {
    switch(it->type)
    {
      case ASSIGN:
      {
        ...
        if(!(code.lhs().id()==ID_symbol && // ignore state-saving
             has_prefix(code.lhs().get_string(ID_identifier), 
                        termination_prefix)))
...
{  
  ...
  {
    if(it->is_goto() && it->guard.id()==ID_or &&
       it->guard.operands().size()==2 &&
       it->guard.op0().id()==ID_symbol &&
       has_prefix(it->guard.op0().get_string(ID_identifier), 
                  "termination::copied"))
      copy_goto=it;
