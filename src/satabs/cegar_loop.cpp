  ...
  abstract_counterexamplet &abstract_counterexample)
{
  ...
  if(save_bps)
  {
    ...
    modelchecker_boolean_programt model_checker_boolean_program(
      ...
      modelchecker_boolean_programt::BOPPO, 0, 0 /* Specifying no concurrency here -- if concurrency is required we need to get it from the command line */);
...
int cegar_loopt::go()
{
...

  {
    ...
    initial_abstraction.build(concrete_model, abstractor.abstract_model, false);
